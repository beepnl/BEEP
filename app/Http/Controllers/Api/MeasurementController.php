    ...
    public function data(Request $request)
    {
        ...
        if($resolution != null)
        {
            if ($device)
            {
                ...
                $queryList         = Device::getAvailableSensorNamesFromData($names, 'sensors', '("key" = \''.$device->key.'\' OR "key" = \''.strtolower($device->key).'\' OR "key" = \''.strtoupper($device->key).'\') AND time >= \''.$staTimestampString.'\' AND time <= \''.$endTimestampString.'\'', $limit, true);
            ...
            if ($location && isset($location->coordinate_lat) && isset($location->coordinate_lon))
            {
                $queryListWeather     = Device::getAvailableSensorNamesFromData($names, 'weather', '"lat" = \''.$location->coordinate_lat.'\' AND "lon" = \''.$location->coordinate_lon.'\' AND time >= \''.$staTimestampString.'\' AND time <= \''.$endTimestampString.'\'', $limit, true);
