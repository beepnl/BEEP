      ...
      template:
          ...
          '<div class="hive" ng-if="mobile == false && new == false">'+
            ...
            '<div class="info">'+
              ...
              '<a ng-if="hive.queen.color != null && hive.queen.color != \'\' && hive.editable" href="#!/hives/{{hive.id}}/edit" class="queen-icon" style="background-color: {{hive.queen.color}};" title="{{hive.queen.name}}"></a>'+
              '<div ng-if="hive.queen.color != null && hive.queen.color != \'\' && !hive.editable" href="#!/hives/{{hive.id}}/edit" class="queen-icon" style="background-color: {{hive.queen.color}};" title="{{hive.queen.name}}"></div>'+
              ...
              '</a>'+
            '</div>'+
            '<a ng-if="hive.id && hive.editable" href="#!/hives/{{hive.id}}/edit" title="{{ lang.edit }}">'+
            ...
            '</a>'+
            '<div ng-if="hive.id && !hive.editable">'+
              '<p class="lid" style="width: {{hive.width}}px;"></p>'+
              '<p ng-repeat="(key, layer) in hive.layers | orderBy : \'-type\' " class="layer" ng-class="layer.type" style="background-color: {{hive.color}}; width: {{hive.width}}px;">'+
                '<span ng-repeat="(key, frame) in layer.frames track by $index" class="frame" ng-class="layer.type"></span>'+
              '</p>'+
            ...
            '<div class="btn-group" role="group" style="margin-bottom: 10px;">'+
              ...
              '<a ng-if="hive.editable" href="#!/hives/{{hive.id}}/inspect" class="btn btn-default" title="{{lang.inspect}}"><i class="fa fa-pencil"></i></a>'+
            '</div>'+
          '</div>'+
          ...
          '<div ng-if="mobile == true && new == false" class="row">'+
            ...
            '<div class="col-xs-2 text-right">'+
              '<a href="#!/hives/{{hive.id}}/inspections" class="btn btn-default" title="{{lang.Inspections}}"><i class="fa fa-search"></i></a>'+
              '<br><a ng-if="hive.editable" href="#!/hives/{{hive.id}}/inspect" class="btn btn-default" title="{{lang.inspect}}"><i class="fa fa-pencil"></i></a>'+
            '</div>'+
          '</div>',
      scope: {
