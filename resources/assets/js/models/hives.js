...
app.service('hives', ['$http', '$rootScope', 'api', 'settings', function($http, $rootScope, api, settings)
{
	...
	this.toggle_open_loc = function(id)
	{
		...
		if (loc)
		{
			...
			self.open_loc_ids = self.open_loc_ids.getUnique();
			//console.log((loc.open ? 'open' : 'close') + ' loc', loc.id, loc.name, self.open_loc_ids);
	...
	this.locationsHandler = function(e, data)
	{
		...
		//console.log('open_loc_ids', open_loc_ids);
		...
		for (var i = 0; i < self.locations.length; i++) 
		{
			...
			{
				loc.open = true;
				if (self.open_loc_ids.indexOf(loc.id) == -1)
					self.open_loc_ids.push(loc.id);
			}
